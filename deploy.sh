...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=ladis29/multi-server:$SHA
kubectl set image deployments/client-deployment client=ladis29/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=ladis29/multi-worker:$SHA